...
for bandwidthLimit in [False, True]:
    print("_________________________________BL_________________________________")
    for sim in sim_runs:
        ...
        for case in case_range:
            ...
            for tau_setup in tau_setup_all:
                ...
                while True:
                    ...
                    tau_tmp = [tau_config] * n_nodes
                    if it_each_local is not None and not isStable:
                        b = msg_size / bandwidth / it_each_local
                        print("     b =", b)
                        if bandwidthLimit:
                            if msg_size != 0 and node_divergence_rank is not None:
                                for i, index in enumerate(node_divergence_rank):
                                    tau_tmp[index] += int(i * b)
                        else:
                            if msg_size != 0:
                                for i in range(len(tau_tmp)):
                                    tau_tmp[i] += int(i * b)
                    print("Tau for each client:", tau_tmp)
                    for n in range(0, n_nodes):
                        msg = ['MSG_WEIGHT_TAU_SERVER_TO_CLIENT', w_global, tau_tmp[n], is_last_round, prev_loss_is_min]
                        send_msg(client_sock_all[n], msg)
                    ...
                    data = 0
                    for n in range(0, n_nodes):
                        msg, msg_size = recv_msg(client_sock_all[n], 'MSG_WEIGHT_TIME_SIZE_CLIENT_TO_SERVER')
                        data += msg_size
                        ...
                        tau_actual = min(tau_actual, msg[3])  # Take max of tau because we wait for the slowest node
                    ...
                    if not use_w_global_prev_due_to_nan:
                        if control_alg is not None:
                            ...
                            tau_new, node_divergence_rank = control_alg.compute_new_tau(data_size_local_all, data_size_total,
                                                                it_each_local, it_each_global, max_time,
                                                                step_size, tau_config, use_min_loss, msg_size, bandwidth)
                        else:
                    ...
                    trans_delay = data / bandwidth
                    print("Tranmission delay:", trans_delay)
                    total_time_recomputed += it_each_local * tau_actual + it_each_global + trans_delay
                    print("Total time:", total_time_recomputed)
                    ...
                    if use_min_loss:
                        tmp_time_for_executing_remaining = total_time_recomputed + it_each_local * (tau_new + 1) + it_each_global * 2 + trans_delay
                    else:
                        tmp_time_for_executing_remaining = total_time_recomputed + it_each_local * tau_new + it_each_global + trans_delay
                    ...
                    if tmp_time_for_executing_remaining < max_time:
                        ...
                        if use_min_loss:  # Take into account the additional communication round in the end
                            tau_config = int((max_time - total_time_recomputed - 2 * it_each_global - it_each_local - trans_delay) / it_each_local)
                        else:
                            tau_config = int((max_time - total_time_recomputed - it_each_global - trans_delay) / it_each_local)
